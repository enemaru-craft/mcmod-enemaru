...
public class GlowstoneLampBlock extends BlockWithEntity {
    public static final BooleanProperty LIT = Properties.LIT;

    public GlowstoneLampBlock(AbstractBlock.Settings settings) {
        super(settings);
        // デフォルトは消灯
    ...
    protected MapCodec<? extends BlockWithEntity> getCodec() {
        return createCodec(GlowstoneLampBlock::new);
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new GlowstoneLampBlockEntity(pos, state);
    }

    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world.isClient) return null;
        return validateTicker(type, ModBlockEntities.GLOWSTONE_LAMP_ENTITY, GlowstoneLampBlockEntity::tick);
    }
