...
/**
 * チャンネル管理式エンドロッドランプブロック。
 */
public class ManagedEndRodLampBlock extends AbstractManagedLightBlock {
    ...
    public ManagedEndRodLampBlock(Settings settings) {
        super(settings);
    ...
    protected int getDefaultChannel() {
        return LightChannels.END_ROD; // 異なるデフォルトチャンネルを設定
    }
}
